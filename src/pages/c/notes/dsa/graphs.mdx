...
A graph <Eq code="G" /> consists of two finite sets: a nonempty set <Eq code="V \parens{G}" /> of *vertices*, and a set <Eq code="E \parens{G}" /> of *edges*, where each edge is associated with a set consisting of either one or two vertices called its *endpoints*.
...
Instead of <Eq code="E \parens{G}" />, a *directed graph* (or *digraph*) has a set <Eq code="D \parens{G}" /> of *directed edges*. A *mixed graph* can contain both directed and undirected edges.
...
<p><Todo>Define *complete graphs* <Eq code="K_n" />. Make diagrams for them. (Epp, Page 633)</Todo></p>

<p><Todo>Define *complete bipartite graphs* <Eq code="K_{m,n}" />. Make diagrams for them. (Epp, Page 633)</Todo></p>
...
A graph <Eq code="H" /> is a *connected component* of its supergraph <Eq code="G" /> iff <Eq code="H" /> is connected, and no connected subgraph of <Eq code="G" /> is a supergraph of <Eq code="H" /> that also contains vertices and edges not in <Eq code="H" />. <Todo>Can we simplify this definition? Also, we should draw diagrams for this.</Todo>

The *degree* of a vertex (denoted <Eq code="\mathrm{deg}\parens{v}" /> for the degree of vertex <Eq code="v" />) is the number of edges incident on it, with loops counted twice. The *total degree* of a graph is the sum of the degrees of all vertices in the graph.
...
- *Discrete Mathematics with Applications* (<Eq code="\Nth{4}{th}" /> edition) by Susanna S. Epp, Chapter 10
