...
<DisplayLatex code="
    \begin{gathered}
        \begin{aligned}
            \left({c_1 + 1}\right) &= 2 \left({i + 1}\right) \\
            c_1 + 1 &= 2 i + 2 \\
            c_1 &= 2 i + 1
        \end{aligned}
        \qquad\qquad
        \begin{aligned}
            \left({c_2 + \xcancel{1}}\right) &= \left({c_1 + \xcancel{1}}\right) + 1 \\
            c_2 &= c_1 + 1
        \end{aligned}
        \qquad\qquad
        \begin{aligned}
            p + 1 &= \left\lfloor{\frac{i + 1}{2}}\right\rfloor \\
            p &= \left\lfloor{\frac{i + 1 - 2}{2}}\right\rfloor
...
Both algorithms have <Latex code="O(\log n)" /> runtime.
...
<DisplayLatex code="
    \text{Pop}() \to \textit{item}
    \qquad\qquad\qquad
    \text{Sift-Down}(\textit{node})
" />
...
<DisplayLatex code="
    \text{Push-Pop}(\textit{item}) \to \textit{item}
    \qquad\qquad\qquad
    \text{Pop-Push}(\textit{item}) \to \textit{item}
" />
...
*Push-pop* and *pop-push* are typically more efficient than using the individual *push* and *pop* operations together. Both operations have <Latex code="O(\log n)" /> runtime.
...
The *heapify* operation converts an arbitrary *complete binary tree* into a valid heap. The algorithm simply runs *sift-down* on array nodes <Latex code="[\left\lfloor{n / 2}\right\rfloor, \dots, 1, 0]" />, in that order.

This algorithm runs in <Latex code="O(n)" />.
