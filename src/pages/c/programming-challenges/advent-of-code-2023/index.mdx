...
Same as with Part 1, let's begin with two hailstone trajectories, though the vectors are now in <Latex code="\Reals^3" />. We use the same time variable now since we need the actual hailstone position at a point in time.
...
The rock trajectory will be defined the same way, but with an <Latex code="x" /> subscript:
