...
This took a very unnecessarily long path towards reaching a successful Python solution that started with a naive BFS solution. That TLE'd, so I tried multiple A\* variants which also TLE'd. When I circled back around to trying BFS with a simple optimization that reduced overhead, that one passed.
