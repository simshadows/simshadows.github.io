...
[**Built-In Functions**](https://docs.python.org/3/library/functions.html), [**Built-In Exceptions**](https://docs.python.org/3/library/exceptions.html#concrete-exceptions), [**Data Model**](https://docs.python.org/3/reference/datamodel.html)<br />
[**`functools`**](https://docs.python.org/3/library/functools.html), [**`itertools`**](https://docs.python.org/3/library/itertools.html), [**`heapq`**](https://docs.python.org/3/library/heapq.html)
...
[**`list`, `tuple`, `range`**](https://docs.python.org/3/library/stdtypes.html#sequence-types-list-tuple-range), [**`str`**](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str)
...
"foobar.txt".startswith("foo")  # --> True
"foobar.txt".endswith(".txt")   # --> True
```
...
[**Classes (Tutorial)**](https://docs.python.org/3/tutorial/classes.html), [**Data Model - Special method names**](https://docs.python.org/3/reference/datamodel.html)
...
[**`os`**](https://docs.python.org/3/library/os.html), [**`sys`**](https://docs.python.org/3/library/sys.html), [**`pathlib`**](https://docs.python.org/3/library/pathlib.html)
...
with open("foobar.txt", "r", encoding=ENCODING) as f:
    s = f.read()  # read full contents into a string
with open("foobar.txt", "r", encoding=ENCODING) as f:
    lines = f.readlines()  # read full contents as list of lines

with open("foobar.txt", "w", encoding=ENCODING) as f:
    for s in lines:
        f.write(s)
```
...
root_path = Path(__file__).resolve().parent  # dir of current module

p = root_path / "barbaz" / "foobar.txt"
if p.is_file():
    s = p.read_text()  # read full contents into a string
    p.write_text(s)    # write (overwrites if file exists)

p = root_path / "barbaz"
if p.is_dir():
    (p / "dead" / "beef").mkdir(parents=True)
    (p / "dead" / "beef").rmdir()
    (p / "foobar.txt").unlink(missing_ok=True)  # delete file/symlink
...
def main():
    first_arg = argv[1]
    if not first_arg:
        raise ValueError("Missing first argument.")
