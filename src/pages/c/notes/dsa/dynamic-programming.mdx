...
<DisplayLatex code="
    \begin{gathered}
        m \brackets{0, w} = 0
            %\tag{eq:0-1-knapsack-problem--opt-substructure-1}
            \\
        m \brackets{i, w} = \max\parens{
            m \brackets{i - 1, w - w_i} + v_i, \,
            m \brackets{i - 1, w}
        }
            %\tag{eq:0-1-knapsack-problem--opt-substructure-2}
    \end{gathered}
" />

<Todo>Finish the rest of the notes on the knapsack problem, and implement equation tagging.</Todo>
