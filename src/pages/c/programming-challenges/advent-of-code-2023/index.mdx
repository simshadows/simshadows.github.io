...
def run(f):
    ...
    for s in f.readlines():
        s = list(filter(lambda x : x in "1234567890", s.strip()))
        n += int(s[0] + s[-1])
    print(n)
...
def run(f):
    ...
    for i, s in enumerate(x.strip() for x in f.readlines()):
        ...
        s1 = list(filter(lambda x : x in nums, s1))
        ...
        s2 = list(filter(lambda x : x in nums, s2))
...
def run(f):
    ...
    for i, s in enumerate(f.readlines()):
        ...
        if all((int(d.get("red", 0)) <= R_LIMIT and int(d.get("green", 0)) <= G_LIMIT and int(d.get("blue", 0)) <= B_LIMIT) for d in sets):
