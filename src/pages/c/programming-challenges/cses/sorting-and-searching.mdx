...
To gain an intuition for going on here, let's try a skewed distribution <Eq code="\brackets{1, 2, 2, 4, 8, 9, 10}" />. The median is calculated by the `len(sticks) // 2` code. Calculating this by hand: <Eq code="\floor{7 / 2} = \floor{3.5} = 3" />. Sketching out the diagram:
...
But what about an even-numbered distribution? Let's try <Eq code="1, 2, 2, 4, 9, 10" />. The ~~median~~ middle is calculated as <Eq code="\floor{6 / 2} = 3" />. Sketching out the diagram:
