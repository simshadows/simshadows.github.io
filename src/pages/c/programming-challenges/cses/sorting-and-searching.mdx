...
But at least I know the idea works, leading me to my first successful solution that passed all tests. I decided to rewrite in C++ since I wasn't having any luck in Python. This solution has time complexity <Eq code="O(m \log n)" /> due to <Eq code="O(\log n)" /> operations on `std::multiset`:
